...
import { ChevronLeft, Bell, Play, LayoutGrid, BarChart2, Settings, X, Activity } from 'lucide-react';
    ...
    <div style={{ backgroundColor: '#F8F9FA', minHeight: '100vh', fontFamily: "'Inter', -apple-system, sans-serif", paddingBottom: '80px' }}>
      ...
      <div style={{ padding: '24px' }}>
        <div style={{ marginBottom: '24px' }}>
          {/* Tenta usar o nome dinâmico se a API já estiver retornando o JOIN, senão cai pro ID */}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {processos.map((proc) => (
            ...
            >
              {/* Esquerda: Ordem + Textos */}
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                {/* Caixa de Ordem (Fundo Laranja Claro, baseado no esquema de alertas suaves) */}
                <div style={{ 
                ...
                <div>
                  ...
                  <p style={{ margin: '4px 0 0 0', fontSize: '13px', color: '#6B7280', display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <Activity size={14} /> {proc.maquina_sugerida}
                  </p>
                </div>
              </div>

              {/* Direita: Botão de Iniciar */}
              <button 
      ...
      {/* JANELA FLUTUANTE (MODAL CUSTOMIZADO) */}
      {modalAberto && (
        <div style={{
          ...
          <div style={{ 
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              ...
              <button onClick={() => setModalAberto(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9CA3AF', padding: 0 }}>
                <X size={24} />
              </button>
            </div>
            
            <div style={{ backgroundColor: '#F3F4F6', padding: '12px', borderRadius: '12px', marginBottom: '20px' }}>
              <span style={{ fontSize: '12px', color: '#6B7280', fontWeight: '600', textTransform: 'uppercase' }}>Operação Atual</span>
              <p style={{ margin: '4px 0 0 0', fontSize: '15px', color: '#111827', fontWeight: '600' }}>{processoSelecionado?.nome_operacao}</p>
            </div>
            
            <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', color: '#374151', marginBottom: '8px' }}>
              ID do Operador (Crachá)
            </label>
            <input 
              type="number" 
              value={operadorId}
            ...
            <button 
              ...
              style={{ 
                width: '100%', padding: '16px', backgroundColor: '#007A33', color: 'white', 
                border: 'none', borderRadius: '14px', fontWeight: '700', fontSize: '15px',
                cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px',
                boxShadow: '0 4px 12px rgba(0, 122, 51, 0.2)'
              }}
            >
